...
| oscanner | ✅ | installed usage banner (no `--help`/man; `-h` errors, banner prints instead), Oracle Scanner 1.0.6 | `-s/-f/-P/-v/-r` confirmed against the real (terse) usage banner; `-r` confirmed required from the usage line itself |
| sidguesser | ✅ | installed usage banner (no `--help`/man), SIDGuesser v1.0.5 | `-i/-d/-p/-r/-m findfirst\|findall` confirmed against the real usage banner; binary is `sidguess` (package name `sidguesser`) |
| tnscmd10g | ✅ | installed usage banner (no `--help`/man) | `version/status/ping` positional commands + `-h/-p/--logfile/--indent/--10G/--rawcmd/--cmdsize` confirmed against the real usage banner; command must render immediately after the program name (modelled via `subcommand=`, same mechanism as netexec/gobuster) — confirmed this matters by checking the app's own fixed slot order would otherwise put flags before the command |
| sqlninja | ✅ | installed `-h` usage output + `/usr/share/doc/sqlninja/README` + bundled `sqlninja.conf.example.gz` | `-m <mode>/-f/-p/-w/-v` and all mode letters (t/f/b/e/x/u/s/k/r/d/i/c/m) confirmed against real `-h` output; config-file-driven interface (no target flag) confirmed from the tool's own usage text and example config |
| mdbtools | ✅ | installed `--help` for `mdb-ver`/`mdb-tables`/`mdb-schema`/`mdb-export`/`mdb-sql` (no man pages), mdbtools 1.0.1 | flags for all 4 wired actions (ver/tables/schema/export) confirmed against real `--help` per binary; positional-vs-flag shape for the table name correctly differs between mdb-schema (`-T`) and mdb-export (positional) per their real usage lines |
| sqlsus | ✅ | installed `--help` (no man page) + `/usr/share/doc/sqlsus/README` + a locally-generated example config (`-g`), sqlsus 0.7.2 | `-g/-e` + `[options] [config file]` order confirmed against real `--help`; config file structure (`url_start`/`url_end`/`blind`/`blind_string`) confirmed from an actually-generated template, not just docs. Internal shell commands (`get_databases`/`get_tables`/`get_current_user`) are well-established public knowledge (sqlsus's own documented command set), not independently re-verified against a live authenticated session in this pass — same caveat as msfconsole's internal grammar |
| theharvester | ✅ | installed `--help` (no man page), theHarvester 4.11.1 | `-d/-b/-l/-c/-r/-t/-f` confirmed against real `--help`; source list (crtsh, duckduckgo, urlscan, etc.) and which need API keys confirmed by reading `/etc/theHarvester/api-keys.yaml` directly on this box, not assumed |
| exiftool | ✅ | installed `man exiftool` (no `--help` beyond a one-liner pointing at the man page), ExifTool 13.55 (package `libimage-exiftool-perl`) | `-r/-gps:all/-common/-json[=FILE]/-c` confirmed against the real man page's Option Overview + Option Details sections; `-json=FILE` (writes directly, no shell redirection needed) confirmed from the man page's bracket syntax, not assumed |
